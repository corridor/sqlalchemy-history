...
class TestValidityStrategyMultithreaded(TestCase):
    ...
    def test_single_objects(self):
        threads = [
            self.WrappedThread(target=self._insert_update_single_article) for n in range(0, NUM_THREADS)
        ]
    ...
    def test_multiple_objects(self):
        threads = [
            self.WrappedThread(target=self._insert_update_multiple_articles) for n in range(0, NUM_THREADS)
        ]
