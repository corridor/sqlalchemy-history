...
from sqlalchemy_history._typing import _O
...
class Operation:
    ...
    def __init__(self, target: _O, type_: int) -> None:
        self.target = target
        self.type = type_
...
class Operations:
    ...
    def add_insert(self, target: _O) -> None:
        if target in self:
    ...
    def add_update(self, target: _O) -> None:
        state_copy = copy(sa.inspect(target, raiseerr=True).committed_state)
    ...
    def add_delete(self, target: _O) -> None:
